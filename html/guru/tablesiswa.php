...
if (isset($_SESSION['user_id'])) {
  ...
  // Fetch classes where the current user is the 'walkes'
  $queryClass = tampildata("SELECT id_walkes, id FROM tbl_classes WHERE id_walkes='$user_id'");

  // Fetch all students and their associated classes
  $data = mysqli_query($koneksi, "SELECT s.*, k.* FROM tbl_siswas s JOIN tbl_classes k ON s.id_class = k.id");
  $totaldata = mysqli_num_rows($data);
} else {
  // Handle cases where user_id is not set
...
if ($queryClass) {
  // Loop through each class and fetch associated students
  foreach ($queryClass as $row) {
    $id_class = $row['id'];

    // Query to get students from the current class
    $studentData = mysqli_query($koneksi, "SELECT s.name_siswa, s.sex, s.fathername, s.mothername,s.status, s.id_class, s.email, s.real_password, s.image, s.id as id_siswa, k.* FROM tbl_siswas s JOIN tbl_classes k ON s.id_class = k.id where s.id_class='$id_class'");
    if ($studentData) {
...
// Now $result contains all students for classes where the current user is 'walkes'
?>
